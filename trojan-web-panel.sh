...
if test -s /etc/nginx/nginx.conf; then
  ...
  sed -i "s/localhost/$your_domain/;" /etc/nginx/nginx.conf
	...
	green "================================="
	sleep 2s
  sed -i '/ExecStart/s/trojan web -p 444/trojan web/g' /etc/systemd/system/trojan-web.service
  sed -i '/ExecStart/s/trojan web/trojan web -p 444/g' /etc/systemd/system/trojan-web.service
  ...
  green " "
	...
	green " "
	 ...
	 blue "  面板管理地址 http://$your_domain:444 "
	green "=================================================================="
else
	green "==============================="
	  red "     Nginx未正确安装 请重试"
